...
from FAST.fast_dataset.utils import get_msra_ann as get_ann
...
from FAST.fast_dataset.utils import scale_aligned_short
...
class FAST_MSRA(data.Dataset):
    def __init__(
        ...
        if split == "train":
            data_dirs = [msra_train_data_dir, hust_train_data_dir] * repeat_times
            gt_dirs = [msra_train_gt_dir, hust_train_gt_dir] * repeat_times
        elif split == "test":
        ...
        for data_dir, gt_dir in zip(data_dirs, gt_dirs):
            img_names = [
                img_name
                for img_name in mmcv.utils.scandir(data_dir)
    ...
    def prepare_train_data(self, index):
        ...
        if self.is_transform:
            img = random_scale(img, self.short_size, scales=[0.5, 2.0], aspects=None)
    ...
    def prepare_test_data(self, index):
        ...
        img = get_img(img_path, self.read_type)
        # if filename == 'IMG_2120' and img.shape[0] < img.shape[1]:
