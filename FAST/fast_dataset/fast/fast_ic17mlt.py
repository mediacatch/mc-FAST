...
from FAST.fast_dataset.utils import shrink
from FAST.fast_dataset.utils import get_img
from FAST.fast_dataset.utils import get_ic17mlt_ann as get_ann
from FAST.fast_dataset.utils import random_scale, random_horizontal_flip, random_rotate
from FAST.fast_dataset.utils import random_crop_padding_v2 as random_crop_padding
from FAST.fast_dataset.utils import update_word_mask, get_vocabulary
from FAST.fast_dataset.utils import scale_aligned_short, center_crop


mlt_root_dir = "./data/ICDAR2017-MLT/"
mlt_train_data_dir = mlt_root_dir + "ch8_training_images/"
mlt_train_gt_dir = mlt_root_dir + "ch8_training_localization_transcription_gt_v2/"
mlt_valid_data_dir = mlt_root_dir + "ch8_validation_images/"
mlt_valid_gt_dir = mlt_root_dir + "ch8_validation_localization_transcription_gt_v2/"
...
class FAST_IC17MLT(data.Dataset):
    def __init__(
        ...
        self.img_size = (
            img_size
            if (img_size is None or isinstance(img_size, tuple))
        ...
        if split == "train":
            if use_val == False:
                data_dirs = [mlt_train_data_dir] * repeat_times
                gt_dirs = [mlt_train_gt_dir] * repeat_times
            else:
                data_dirs = [mlt_train_data_dir, mlt_valid_data_dir] * repeat_times
                gt_dirs = [mlt_train_gt_dir, mlt_valid_gt_dir] * repeat_times
        elif split == "valid":
            data_dirs = [mlt_valid_data_dir]
            gt_dirs = [mlt_valid_gt_dir]
        else:
            print("Error: split must be train or valid!")
            raise
    ...
    def prepare_train_data(self, index):
        ...
        if self.is_transform:
            img = random_scale(
                img, self.short_size, scales=[0.7, 1.3], aspects=[0.9, 1.1]
            )
        ...
        if bboxes.shape[0] > 0:
            bboxes = np.reshape(
                bboxes * ([img.shape[1], img.shape[0]] * 4), (bboxes.shape[0], -1, 2)
            ).astype("int32")
        ...
        if self.is_transform:
            ...
            img, gt_instance, training_mask, gt_kernel = (
                imgs[0],
                imgs[1],
                imgs[2],
                imgs[3],
