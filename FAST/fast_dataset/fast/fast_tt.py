...
from FAST.fast_dataset.utils import get_tt_ann as get_ann
...
class FAST_TT(data.Dataset):
    def __init__(
        ...
        if split == "train":
            data_dirs = [tt_train_data_dir] * repeat_times
            gt_dirs = [tt_train_gt_dir] * repeat_times
        elif split == "test":
    ...
    def prepare_train_data(self, index):
        ...
        if len(bboxes) > 0:
            for i in range(len(bboxes)):
                bboxes[i] = np.reshape(
                    bboxes[i]
                    * ([img.shape[1], img.shape[0]] * (bboxes[i].shape[0] // 2)),
