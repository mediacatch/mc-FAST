...
from FAST.fast_dataset.utils import get_synth_ann as get_ann
...
class FAST_Synth(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        gt_words = np.full(
            (self.max_word_num, self.max_word_len), self.char2id["PAD"], dtype=np.int32
        )
        ...
        if self.is_transform:
            img = random_scale(
                img, self.short_size, scales=[0.5, 2.0], aspects=[0.9, 1.1]
            )
