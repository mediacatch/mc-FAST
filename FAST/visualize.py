from FAST.fast_dataset.fast.fast_msra import msra_test_data_dir, msra_test_gt_dir
from FAST.fast_dataset.fast.fast_ctw import ctw_test_data_dir, ctw_test_gt_dir
from FAST.fast_dataset.fast.fast_tt import tt_test_data_dir, tt_test_gt_dir
from FAST.fast_dataset.fast.fast_ic15 import ic15_test_data_dir, ic15_test_gt_dir
from FAST.fast_dataset.utils import (
    get_msra_ann,
    get_ctw_ann,
    get_tt_ann,
    get_ic15_ann,
...
def visual(get_ann, data_dir, gt_dir, pred_dir, dataset):
    ...
    for index, (img_path, gt_path, pred_path) in tqdm(
        enumerate(zip(img_paths, gt_paths, pred_paths)), total=len(img_paths)
    ):
        ...
        try:  # process annotations
            if dataset == "msra":
                gt = np.reshape(
                    gt * ([img.shape[1], img.shape[0]] * 4), (gt.shape[0], -1, 2)
                ).astype("int32")
            elif dataset == "ctw":
                for i in range(len(gt)):
                    gt[i] = np.reshape(
                        gt[i] * ([img.shape[1], img.shape[0]] * (gt[i].shape[0] // 2)),
                        (gt[i].shape[0] // 2, 2),
                    ).astype("int32")
            elif dataset == "tt":
                for i in range(len(gt)):
