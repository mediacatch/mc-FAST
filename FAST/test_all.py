from FAST.utils import ResultFormat
import subprocess
import argparse
import json
import os
...
def ctw_eval(ep):
    run_cmd = "python test.py %s %s/checkpoint_%dep.pth.tar --min-score 0" % (
        args.config,
        args.checkpoint,
        ep,
    )
    ...
    for i in range(80, 90 + 1):
        generate_results_from_json(
            min_score=i / 100, dataset="CTW", result_path="outputs/submit_ctw/"
        )
        eval_cmd = cd_root_cmd + " && " + "cd eval && sh eval_ctw.sh"
        p = subprocess.Popen(
            eval_cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT
        )
        p.wait()
        f = 0
        line = ""
        ...
        p.stdout.close()
    print(
        "best f-measure %f at ep %d with min-score %.2f"
        % (best_f, best_ep, best_min_score)
    )
    print(best_line)
    return max(f_list)
...
def ic15_eval(ep):
    ...
    for i in range(80, 90 + 1):
        generate_results_from_json(
            min_score=i / 100, dataset="IC15", result_path="outputs/submit_ic15.zip"
        )
...
def tt_eval(ep):
    ...
    for i in range(80, 90 + 1):
        generate_results_from_json(
            min_score=i / 100, dataset="TT", result_path="outputs/submit_tt/"
        )
...
def msra_eval(ep):
    ...
    for i in range(80, 95 + 1):
        generate_results_from_json(
            min_score=i / 100, dataset="MSRA", result_path="outputs/submit_msra/"
        )
