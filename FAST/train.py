...
from FAST.fast_dataset import build_data_loader
from FAST.models import build_model
from FAST.utils import AverageMeter, setup_logger, EMA
from FAST.fast_dataset.dataloader import DataLoaderX
...
def train(train_loader, model, optimizer, ema, epoch, start_iter, cfg):
    ...
    for iter, data in enumerate(train_loader):
        ...
        if iter % 10 == 0:
            output_log = (
                "({batch}/{size}) LR: {lr:.6f} | Batch: {bt:.3f}s | Total: {total:.0f}min | "
...
def save_checkpoint(state, checkpoint_path, cfg):
    ...
    if cfg.data.train.type in ["synth"] or (
        state["iter"] == 0
        and state["epoch"] > cfg.train_cfg.epoch - 100
        and state["epoch"] % cfg.train_cfg.save_interval == 0
    ):
...
def main(args):
    ...
    if cfg.train_cfg.optimizer == "SGD":
        optimizer = torch.optim.SGD(
            model.parameters(), lr=cfg.train_cfg.lr, momentum=0.9, weight_decay=1e-4
        )
...
if __name__ == "__main__":
    ...
    setup_logger(
        ...
        save_dir=os.path.join(
            "checkpoints", os.path.basename(args.config).replace(".py", "")
        ),
        distributed_rank=0,
        mode="a+",
    )
